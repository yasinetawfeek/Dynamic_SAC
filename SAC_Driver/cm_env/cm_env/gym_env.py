    ...
    def step(self, action):
        ...
        if REWARD_FUNCTION_TO_USE == 0:
            ...
            if FUNCTION_4_RANDOM_POINT:
                target = self.track.random_point_in_quadrilateral(self.blue_cones_detected[2], self.blue_cones_detected[3], self.yellow_cones_detected[2], self.yellow_cones_detected[3])
            else:
                target = self.track.find_middle_point(self.blue_cones_detected[2], self.blue_cones_detected[3], self.yellow_cones_detected[2], self.yellow_cones_detected[3])
        ...
        if REWARD_FUNCTION_TO_USE == 5:
            ...
            if is_outside_track or ((time.time() - self.episode_timer > EPISODE_TIME_LIMIT) and EPISODE_TIME_LIMIT != 0):
                reward = -abs(FUNCTION_5_PENALTY_FOR_EXIT)
